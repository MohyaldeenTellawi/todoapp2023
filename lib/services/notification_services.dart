...
class NotifyHelper {
  ...
  tz.TZDateTime _nextInstanceOfTenAM(int hour, int minutes,int remind,String repeat,String date) {
    ...
    tz.TZDateTime scheduledDate = tz.TZDateTime(tz.local, fd.year, fd.month, fd.day, hour, minutes);
    print('scheduledDate $scheduledDate');

    scheduledDate = afterRemind(remind, scheduledDate);
    if (scheduledDate.isBefore(now)) {
      if(repeat=='Daily'){
        tz.TZDateTime(tz.local, now.year, now.month, (formattedDate.day)+1, hour, minutes);
      }
      if(repeat=='Weekly'){
        scheduledDate =
        tz.TZDateTime(tz.local, now.year, now.month, (formattedDate.day)+7, hour, minutes);
      }
      if(repeat=='Monthly'){
        scheduledDate =
        tz.TZDateTime(tz.local, now.year, (formattedDate.month)+1, formattedDate.day, hour, minutes);
      }
      scheduledDate = afterRemind(remind, scheduledDate);
    }
    ...
    return scheduledDate;
     ...
     afterRemind(int remind, tz.TZDateTime scheduledDate) {
     if(remind==5){
